...
class DataBuild:
	...
	def transformData(self,imagepaths,structobj):
		...
		for i in range(structobj.amt):
            ...
            label = image_path[85:87] # get the label from the whole path     #changes happen here
			...
			new_file_path = "{}/{}_{}_{}.jpg".format(structobj.path_dest,structobj.name,label,i)  # changes happen here 
...
if __name__ == '__main__':
	...
	#path = '/home/hscilab282-10/Desktop/Xi_Zhou/AI/11_6_2018_CNN/CNN_Letter/'

	path = os.getcwd()
	print("path:",path)

	root = path +"/Letters_resize_gs/"
	print("root:",root)
	file_list = os.listdir(root) 

	for i in range(len(file_list)):
		print(file_list[i])
		letter = DataStruct(name= file_list[i].split(".")[0] ,
							path_src="{}/Letters_resize_gs/{}".format(path, file_list[i]),   # raw_letters are resized and grayscaled images 
							path_dest="{}/All_Letters_Data".format(path),
							amt=200)

		# brilliant to build the directory to different folders and save there
							   
		#lettertest = DataStruct(name = file_list[i] , 
		#	                        path_src="{}/Letters_resize_gs".format(path),
		#	                       path_dest="{}/test_data".format(path),  # put all images into a single folder
		#	                       amt=200)                                

		buildobj = DataBuild((letter,))   # must be a tuple
		buildobj.run()
