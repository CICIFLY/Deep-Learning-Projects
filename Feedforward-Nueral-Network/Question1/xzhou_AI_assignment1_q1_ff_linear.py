...
for j in range(40000):
    #layers
    l0 = x #inputs
    l1 = identity_function(np.dot(l0,syn0)) #hidden layer
    l2 = identity_function(np.dot(l1,syn1)) #output
    #back propagation
    l2_error = y_normalized - l2
    if(j % 10000) == 0:
        print ('Error '+str(np.mean(np.abs(l2_error))))
    l2_delta = l2_error*identity_function(l2,deriv=True)
    l1_error = l2_delta.dot(syn1.T)
    l1_delta = l1_error*identity_function(l1,deriv=True)
